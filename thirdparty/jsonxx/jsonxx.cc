...
bool parse_array(std::istream &input, Array &array) { return array.parse(input); }

bool parse_object(std::istream &input, Object &object) { return object.parse(input); }
...
bool parse_value(std::istream &input, Value &value) { return value.parse(input); }
...
std::string tag(unsigned format, unsigned depth, const std::string &name, const jsonxx::Value &t) {
    ...
    case jsonxx::Value::ARRAY_:
        ss << "[\n";
        for (Array::container::const_iterator it = t.array_value_->values().begin(), end = t.array_value_->values().end(); it != end; ++it)
    ...
    case jsonxx::Value::OBJECT_:
        ss << "{\n";
        for (Object::container::const_iterator it = t.object_value_->kv_map().begin(), end = t.object_value_->kv_map().end(); it != end; ++it)
...
const std::map<std::string, Value *> &Object::kv_map() const { return value_map_; }
std::string Object::write(unsigned format) const { return format == JSON ? json() : xml(format); }
void Object::reset() {
...
Object Object::duplicate() const {
    ...
    for (auto &kv : value_map_) {
        ret << kv.first << (kv.second->duplicate());
    }
    return ret;
}
...
Value Value::duplicate() const {
    ...
    case NUMBER_:
        return Value(number_value_);
    case STRING_: {
        Value ret;
        ret.type_ = STRING_;
        ret.string_value_ = new String(*string_value_);
        return ret;
    }
    case ARRAY_: {
        Value ret;
        ret.type_ = ARRAY_;
        ret.array_value_ = new Array(array_value_->duplicate());
        return ret;
    }
    case OBJECT_: {
        Value ret;
        ret.type_ = OBJECT_;
        ret.object_value_ = new Object(object_value_->duplicate());
        return ret;
    }
    case INVALID_:
        return Value();
...
// template <typename T> const T &Array::get(unsigned int i, const typename identity<T>::type &default_value) const {
//     if (has<T>(i)) {
//         const Value *v = values_.at(i);
//         return v->get<T>();
//     } else {
...
//     }
// }
// const Value Array::get(const unsigned int i) const {
//     JSONXX_ASSERT(has(i));
//     return *(values_.at(i));
// }
...
// template <typename T> const T &Object::get(const std::string &key, const typename identity<T>::type &default_value) const {
//     if (has<T>(key)) {
//         return value_map_.find(key)->second->get<T>();
//     } else {
//         return default_value;
...
//     container::const_iterator it = value_map_.begin(), end = value_map_.end();
//     for (/**/; it != end; ++it) {
//         if (key == it->first) {
//             delete it->second;
//             ++it;
